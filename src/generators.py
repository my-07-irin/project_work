...
def filter_by_currency(list_input: list, currency_input: str) -> Any:
    for info_list in list_input:
        # if info_list.get(["operationAmount"]["currency"]["name"]) == None:
        #     info_list = f'в записи {info_list.get("id")} нет поля с описанием валюты'
        if info_list["operationAmount"]["currency"]["name"] == currency_input:
            yield info_list


def transaction_descriptions(list_input: list) -> Any:
    for info_operation in list_input:
        if info_operation.get("description") is None:
            operation_out = f'в записи {info_operation.get("id")} нет поля: description'
        else:
            operation_out = info_operation.get("description")
        yield operation_out
...
if __name__ == "__main__":
    ...
    usd_transactions = filter_by_currency(list_bank, "USD")
